-- Update join_room to treat rooms inactive for 24h+ as if they don't exist
create or replace function join_room(p_slug text, p_display_name text)
returns table (out_room_id uuid, out_participant_id uuid)
language plpgsql
security definer
as $$
declare
...
create policy "rooms_select_for_join_lookup"
  on rooms for select
  using (last_active_at > now() - interval '24 hours');
